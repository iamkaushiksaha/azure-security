...
```mermaid
gantt
    title Correlated Attack Timeline (T0–T56+)
    dateFormat  X
    axisFormat  T%L

    section Credential Access
    Failed sign-ins (Brute Force / T1110)          :a1, 0, 10

    section Initial Access
    Successful sign-in (Valid Accounts / T1078)    :a2, 11, 1

    section Privilege Escalation
    Privilege change (Account Manipulation / T1098):a3, 12, 9

    section Lateral Movement & Execution
    RDP + PowerShell (Remote Services / T1021)     :a4, 21, 20
    Firewall traffic + scanning (T1046)            :a5, 25, 21

    section Detection & Response
    Alerts generated (Analytics rules)             :a6, 46, 10
    Incident creation (Correlation)                :a7, 56, 2
```
...
```mermaid
flowchart LR
    A[Brute Force<br/>T1110] --> B[Valid Sign-in<br/>T1078]
    B --> C[Account Manipulation<br/>T1098]
    C --> D[RDP Lateral Movement<br/>T1021]
    D --> E[PowerShell Execution<br/>T1059.001]
    E --> F[Network Service Scanning<br/>T1046]
    F --> G[Sentinel Alerts]
    G --> H[Sentinel Incident<br/>(Correlated)]
```
